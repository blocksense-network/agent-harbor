...
fn main() {
    ...
    let get_shared_ptr = unsafe {
        libc::dlsym(
            libc::RTLD_DEFAULT,
            b"__stackable_get_shared_read\0".as_ptr() as *const _,
        )
    };
    let call_real_ptr = unsafe {
        libc::dlsym(
            libc::RTLD_DEFAULT,
            b"__stackable_call_real_read\0".as_ptr() as *const _,
        )
    };
    ...
    if normal_result >= 0 && real_result >= 0 {
        println!("✓ Both calls succeeded");
    } else if normal_result < 0 && real_result >= 0 {
        println!("✓ call_real! bypassed hook that was blocking the normal call");
    } else {
        println!(
            "⚠ Unexpected results: normal={}, real={}",
