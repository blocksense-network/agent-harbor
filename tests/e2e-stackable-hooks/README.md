...
Comprehensive end-to-end tests for the `stackable-hooks` crate, demonstrating priority-based hook ordering.

## Overview

This test crate provides integration tests that verify the `stackable-hooks` library's functionality, including its priority system for ordering multiple hooks:
...
The `stackable-hooks` library supports priority-based hook ordering:
...
- Each hook can call:
  - `stackable_hooks::call_next!()` to continue to the next hook in priority order
  - `stackable_hooks::call_real!()` to bypass all remaining hooks and call the original function directly
