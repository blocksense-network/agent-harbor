...
//! Shim library A for testing priority-based stackable-interpose functionality
...
hook! {
    ...
    unsafe fn read(stackable_self, fd: libc::c_int, buf: *mut libc::c_void, count: libc::size_t)
        ...
        let result = stackable_interpose::call_next!(stackable_self, read, fd, buf, count);
...
hook! {
    ...
    unsafe fn close(stackable_self, fd: libc::c_int) -> libc::c_int => my_close {
        ...
        let result = if use_call_real {
            ...
            stackable_interpose::call_real!(close, fd)
        } else {
