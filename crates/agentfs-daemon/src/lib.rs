...
// Use handshake types from the local crate
use crate::handshake::*;

/// Encode an SSZ message to bytes
pub fn encode_ssz_message(data: &impl ssz::Encode) -> Vec<u8> {
    data.as_ssz_bytes()
}

/// Decode an SSZ message from bytes
pub fn decode_ssz_message<T: ssz::Decode>(data: &[u8]) -> Result<T, ssz::DecodeError> {
...
impl AgentFsDaemon {
    ...
    pub fn handle_watch_request(&self, request: &Request) -> Result<Response, anyhow::Error> {
        match request {
