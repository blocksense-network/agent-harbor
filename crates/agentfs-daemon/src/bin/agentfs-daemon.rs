...
fn main() {
    ...
    if args.len() < 2 {
        ...
        tracing::error!("Backstore modes: InMemory, HostFs, RamDisk");
        std::process::exit(1);
    }
        ...
        match args[i].as_str() {
            "--lower-dir" => {
                if i + 1 < args.len() {
                    ...
                    tracing::error!("--lower-dir requires an argument");
                    std::process::exit(1);
                }
            }
            "--upper-dir" => {
                if i + 1 < args.len() {
                    ...
                    tracing::error!("--upper-dir requires an argument");
                    std::process::exit(1);
                }
            }
            "--work-dir" => {
                if i + 1 < args.len() {
                    ...
                    tracing::error!("--work-dir requires an argument");
                    std::process::exit(1);
                }
            }
            "--backstore-mode" => {
                if i + 1 < args.len() {
                    backstore_mode = match args[i + 1].as_str() {
                        ...
                        _ => {
                            tracing::error!("Invalid backstore mode: {}", args[i + 1]);
                            std::process::exit(1);
                        }
                    };
                    i += 2;
                } else {
                    tracing::error!("--backstore-mode requires an argument");
                    std::process::exit(1);
                }
            }
            "--backstore-root" => {
                if i + 1 < args.len() {
                    ...
                    tracing::error!("--backstore-root requires an argument");
                    std::process::exit(1);
                }
            }
            "--backstore-size-mb" => {
                if i + 1 < args.len() {
                    ...
                    tracing::error!("--backstore-size-mb requires an argument");
                    std::process::exit(1);
                }
            }
            arg => {
    ...
    tracing::info!("AgentFS Daemon: listening on {}", socket_path);
    ...
    let daemon =
        match AgentFsDaemon::new_with_backstore(lower_dir, upper_dir, work_dir, backstore_mode) {
            ...
            Err(e) => {
                tracing::error!("Failed to create AgentFS daemon: {:?}", e);
                std::process::exit(1);
            }
        };

    tracing::info!("AgentFS Daemon: initialized successfully");
    ...
    for stream in listener.incoming() {
        match stream {
            ...
            Err(e) => {
                tracing::error!("AgentFS Daemon: accept error: {}", e);
                break;
            }
        }
    }

    tracing::info!("AgentFS Daemon: shutting down");
}
...
fn handle_client(mut stream: UnixStream, daemon: Arc<Mutex<AgentFsDaemon>>, client_pid: u32) {
    // Register the process with the daemon
    {
        ...
        if let Err(err) = daemon.register_process(client_pid, 0, 0, 0) {
            tracing::error!(
                "AgentFS Daemon: register_process failed for pid {}: {}",
                client_pid,
                err
            );
            return;
        ...
        Ok(request) => {
            match request {
                ...
                Request::Readlink((_version, readlink_req)) => {
                    ...
                    tracing::debug!("AgentFS Daemon: readlink({}, pid={})", path, client_pid);
                    let mut daemon = daemon.lock().unwrap();
                    match daemon.handle_readlink(path, client_pid) {
                        Ok(target) => {
                            tracing::debug!(
                                "AgentFS Daemon: readlink succeeded, target: {}",
                                target
                            );
                        ...
                        Err(e) => {
                            tracing::error!("AgentFS Daemon: readlink failed: {}", e);
                            let response =
                ...
                Request::DirRead((_version, dir_read_req)) => {
                    ...
                    tracing::debug!("AgentFS Daemon: dir_read(handle={})", handle);
                    let mut daemon = daemon.lock().unwrap();
                    match daemon.handle_dir_read(handle, client_pid) {
                        ...
                        Err(e) => {
                            tracing::error!("AgentFS Daemon: dir_read failed: {}", e);
                            let response =
                ...
                Request::DirClose((_version, dir_close_req)) => {
                    ...
                    tracing::debug!("AgentFS Daemon: dir_close(handle={})", handle);
                    let mut daemon = daemon.lock().unwrap();
                    match daemon.handle_dir_close(handle, client_pid) {
                        Ok(()) => {
                            tracing::debug!("AgentFS Daemon: dir_close succeeded");
                            let response = Response::dir_close();
                            send_response(&mut stream, &response);
                        }
                        Err(e) => {
                            tracing::error!("AgentFS Daemon: dir_close failed: {}", e);
                            let response =
                ...
                Request::FdDup((_version, fd_dup_req)) => {
                    ...
                    tracing::debug!("AgentFS Daemon: fd_dup(fd={})", fd);
                    let mut daemon = daemon.lock().unwrap();
                    match daemon.handle_fd_dup(fd, client_pid) {
                        Ok(duped_fd) => {
                            tracing::debug!(
                                "AgentFS Daemon: fd_dup succeeded, new fd: {}",
                                duped_fd
                            );
                        ...
                        Err(e) => {
                            tracing::debug!("AgentFS Daemon: fd_dup failed: {}", e);
                            let response =
                ...
                Request::PathOp((_version, path_op_req)) => {
                    ...
                    tracing::debug!("AgentFS Daemon: path_op(path={}, op={})", path, operation);
                    let mut daemon = daemon.lock().unwrap();
                    match daemon.handle_path_op(path, operation, path_op_req.args, client_pid) {
                        Ok(result) => {
                            tracing::debug!("AgentFS Daemon: path_op succeeded");
                            let response = Response::path_op(result);
                            send_response(&mut stream, &response);
                        }
                        Err(e) => {
                            tracing::debug!("AgentFS Daemon: path_op failed: {}", e);
                            let response =
                ...
                Request::DaemonStateFilesystem(DaemonStateFilesystemRequest { query }) => {
                    ...
                    match daemon.get_daemon_state_filesystem(&query) {
                        ...
                        Err(e) => {
                            tracing::debug!("AgentFS Daemon: filesystem state query failed: {}", e);
                            let response = Response::error(
