...
#[allow(deprecated)] // macOS system header defines mach_header_64; keep for ABI compatibility
...
/// # Safety
/// Caller must ensure `dispatch` points to a valid trampoline inside the current image and that
/// the returned symbol address is used with the correct signature for `symbol`.
pub unsafe fn resolve_original(
    ...
    if ptr.is_null() {
        panic!(
            "stackable-hooks: unable to resolve original symbol {}",
...
/// # Safety
/// Caller must guarantee that `local` is a valid pointer to a symbol with the same signature as
/// `symbol`. Returned pointer must be used following the original symbol’s contract.
