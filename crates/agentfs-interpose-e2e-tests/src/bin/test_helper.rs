...
mod macos_only {
    ...
    fn test_t25_5_directory_operations_with_dirfd(args: &[String]) {
        ...
        if fd2 >= 0 {
            ...
            let c_file = std::ffi::CString::new("file.txt").unwrap();
            let fd3 = unsafe {
                libc::openat(fd2, c_file.as_ptr(), libc::O_CREAT | libc::O_WRONLY, 0o644)
            };
    ...
    fn test_t25_7_link_operations_with_dirfd(args: &[String]) {
        ...
        let link_result =
            unsafe { libc::linkat(fd1, c_source.as_ptr(), fd1, c_hardlink.as_ptr(), 0) };
                ...
                for i in 0..20 {
                    ...
                    if i % 7 == 0 && !fds.is_empty() {
                        ...
                        if dup_fd >= 0 {
                            ...
                            let dup_file_fd = unsafe {
                                libc::openat(dup_fd, c_file_name.as_ptr(), libc::O_RDONLY)
                            };
                            if dup_file_fd >= 0 {
                                println!(
                                    "Thread {}: dup'd fd {} works for openat",
                                    thread_id, dup_fd
                                );
                                unsafe {
        ...
        if read_fd >= 0 {
            ...
            if bytes_read > 0 {
                ...
                if content.contains("lower layer content") {
                    println!(
                        "✓ Content matches lower layer - no copy-up occurred for read operation"
                    );
                } else {
    ...
    extern "C" {
        ...
        fn acl_set_file(path: *const libc::c_char, acl_type: acl_type_t, acl: acl_t)
        -> libc::c_int;
