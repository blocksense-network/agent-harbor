...
impl ViewModel {
            ...
            if let Some(card) = self.draft_cards.get(idx) {
                match card.focus_element {
                    ...
                    CardFocusElement::GoButton => {
                        ...
                        tracing::error!(
                            "🔍 TUI DIRECT GO BUTTON LAUNCH: idx={}, split_mode={:?} (from config)",
                            idx,
                            default_split_mode
                        );
    ...
    pub fn launch_task(
        ...
        if let Some(ref opts) = advanced_options {
            tracing::error!(
                "🔍 TUI LAUNCH_TASK: interactive_mode={}, sandbox_profile={:?}",
                opts.interactive_mode,
                opts.sandbox_profile
            );
        }
        ...
        if let Some(card) = self.draft_cards.get(draft_card_index) {
            ...
            if let Some(options) = &advanced_options {
                tracing::error!(
                    "🔧 TUI LAUNCH_TASK: Applying advanced options to builder (interactive={}, sandbox={:?})",
                ...
                tracing::error!("🔧 TUI LAUNCH_TASK: No advanced options to apply");
            }
    ...
    fn launch_task_with_option(&mut self, draft_id: String, selected_option: String) {
        ...
        if let Some(draft_index) = self.draft_cards.iter().position(|card| card.id == draft_id) {
            ...
            let (split_mode, focus) = match selected_option.as_str() {
                ...
                "Launch in split view (s)" | "Launch in background (b)" => {
                    (ah_core::SplitMode::Auto, false)
                }
            ...
            tracing::error!(
                "🔍 TUI LAUNCH_TASK_WITH_OPTION: advanced_options present? {}",
                advanced_options.is_some()
            );
            if let Some(ref opts) = advanced_options {
                tracing::error!(
                    "🔍 TUI LAUNCH_TASK_WITH_OPTION: interactive_mode={}, sandbox_profile={:?}",
                    opts.interactive_mode,
                    opts.sandbox_profile
                );
            }
...
impl Default for AdvancedLaunchOptions {
    fn default() -> Self {
        Self {
            ...
            // Interactive mode is true by default in TUI context - users can interact with agents
            // Set to false via Launch Options modal if non-interactive execution is desired
