...
impl Settings {
    ...
    // NOTE: Removed save_default_split_mode() function
    // Split mode preferences are now session-only (not persisted to disk).
    // The TUI directly updates self.settings.default_split_mode for the current session,
    // and this resets to the config default when the TUI is restarted.
}
