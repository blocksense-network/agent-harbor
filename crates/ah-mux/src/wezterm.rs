...
impl WezTermMultiplexer {
    ...
    pub fn new() -> Result<Self, MuxError> {
        ...
        // Test CLI connectivity - if this fails, provide helpful error message
        if let Err(e) = Self::test_cli_connectivity() {
            error!("WezTerm CLI connectivity test failed: {}", e);
            return Err(MuxError::ConfigurationError(format!(
                "WezTerm CLI not accessible: {}. \n\nTo enable WezTerm CLI support:\n\
                1. Start WezTerm GUI application\n\
                2. OR add to ~/.config/wezterm/wezterm.lua:\n\
                   config.unix_domains = {{\n\
                     {{\n\
                       name = 'unix',\n\
                       socket_path = '/tmp/wezterm-' .. os.getenv('USER') .. '.sock',\n\
                     }},\n\
                   }}\n\
                   config.default_gui_startup_args = {{ 'connect', 'unix' }}\n\
                3. Then restart WezTerm or run: wezterm connect unix",
                e
            )));
        }
