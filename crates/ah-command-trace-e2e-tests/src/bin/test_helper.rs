...
fn log_step(step: &str) {
    ...
    log_step("start shell_and_interpreter"); (feat(command-tracing): Capture sub-process output)
...
println!("Shell and interpreter subprocess test complete");
    log_step("completed shell_and_interpreter"); (feat(command-tracing): Capture sub-process output)
}
