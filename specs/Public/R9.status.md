...
- `crates/ah-command-trace-shim/src/posix.rs` - Common POSIX hook implementations using the vendored `stackable-hooks` macros
...
- **macOS Interpose Support (Nov 2025 update)**:
  - Vendored `redhook` into `crates/stackable-hooks` and updated every hook definition to use that crate. The macOS path now emits explicit `__DATA,__interpose` entries marked `#[used]`, preventing `-dead_strip` from dropping our trampolines and finally unlocking stdout/stderr chunk capture on macOS while keeping Linux behavior unchanged.
