...
The M0 implementation follows a layered architecture with platform-agnostic core logic and platform-specific adapters:
...
- **Platform Adapters**:
  - macOS (`src/platform/macos.rs`): Uses `DYLD_INSERT_LIBRARIES` for library interposition and performs synchronous handshake during initialization to avoid process blocking.
  - Linux (`src/platform/linux.rs`): Uses `LD_PRELOAD` for library preloading with similar synchronous handshake approach.
- **Communication Protocol**: Uses genuine SSZ (Simple Serialize) binary protocol over Unix domain sockets, with length-prefixed messaging for reliable framing.
- **Test Infrastructure**: Separate `crates/ah-command-trace-e2e-tests` crate prevents shim injection into `cargo test` runner, using synchronous socket servers for reliable testing.
...
- `crates/ah-command-trace-shim/src/platform/linux.rs` - Linux LD_PRELOAD implementation
- `crates/ah-command-trace-proto/src/lib.rs` - SSZ protocol definitions and encoding/decoding
- `crates/ah-command-trace-proto/src/messages.rs` - Request/Response message types
- `crates/ah-command-trace-e2e-tests/src/lib.rs` - Test utilities and platform-specific loaders
- `crates/ah-command-trace-e2e-tests/tests/shim_injection_smoke.rs` - Integration test suite
