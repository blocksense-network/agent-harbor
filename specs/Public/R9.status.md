...
- [x] Scaffold `crates/ah-command-trace-shim` with shared core + platform modules (macOS `.dylib`, Linux `.so`) and build-time gating (`cargo cfg` ensures no-std symbols are resolved before tests run).
- [x] Provide ergonomic loaders for tests:
  - macOS: wrapper around `DYLD_INSERT_LIBRARIES` that spawns arbitrary binaries while guaranteeing SIP-safe environment variables.
  - Linux: helper that wraps commands with `LD_PRELOAD`, handles musl/glibc differences, and skips static binaries gracefully.
- [x] Add `tests/shim_injection_smoke.rs` integration suite:
  - Launches a trivial helper binary (`src/bin/test_helper.rs`) via both loaders.
...
- [x] Document loader environment variables (`AH_CMDTRACE_ENABLED`, `AH_CMDTRACE_SOCKET`, `AH_CMDTRACE_LOG`) and safe defaults (disabled when socket unset).

**Verification**

- [x] `cargo test -p ah-command-trace-e2e-tests shim_injection_smoke` runs on macOS + Linux CI lanes.
- [x] Negative test ensures shim stays dormant when `AH_CMDTRACE_ENABLED=0`.
- [x] Sanitizer builds (ASAN/TSAN) run weekly to catch unsafe init ordering issues (esp. before `main()`).

**Implementation Details**

The M0-M1 implementation follows a layered architecture with platform-agnostic core logic and platform-specific adapters:

- **Core Layer** (`crates/ah-command-trace-shim/src/lib.rs`): Contains initialization state management, environment variable parsing, and shared types. Uses `OnceCell` for global state and provides conditional logging based on `AH_CMDTRACE_LOG`.
- **Platform Adapters**:
  - macOS (`src/platform/macos.rs`): Uses `DYLD_INSERT_LIBRARIES` for library interposition and performs synchronous handshake during initialization to avoid process blocking. Interposes `fork`, `vfork`, `execve`, `execvp`, and `posix_spawn` system calls to capture subprocess creation.
  - Linux (`src/platform/linux.rs`): Uses `LD_PRELOAD` for library preloading with similar synchronous handshake approach. Interposes `fork`, `vfork`, `clone`, `execve`, `execvp`, and `posix_spawn` system calls.
- **Communication Protocol**: Uses genuine SSZ (Simple Serialize) binary protocol over Unix domain sockets, with length-prefixed messaging for reliable framing. Extended `Request` enum to include `CommandStart` messages containing PID, PPID, executable, arguments, environment, and working directory.
- **Test Infrastructure**: Separate `crates/ah-command-trace-e2e-tests` crate prevents shim injection into `cargo test` runner, using the `ah-command-trace-server` crate's `TestServer` for reliable async testing with proper concurrency and shutdown handling. Added `spawn_tree` test helper that creates process trees with various launch patterns.
- **Edge Case Handling**: All fork/exec sequences are captured, even for short-lived commands. Zombie processes are not interfered with. PPID is captured at exec time. Shim injection is skipped when `AT_SECURE` is set (secure execution mode). Failed exec attempts are still recorded with intended executable path.

**Key Source Files**

- `crates/ah-command-trace-shim/src/lib.rs` - Core initialization and state management
- `crates/ah-command-trace-shim/src/core.rs` - Shared state and initialization logic
- `crates/ah-command-trace-shim/src/posix.rs` - Common POSIX hook implementations using the vendored `stackable-interpose` macros
- `crates/ah-command-trace-shim/src/platform/macos.rs` - macOS DYLD interposition implementation
- `crates/ah-command-trace-shim/src/platform/linux.rs` - Linux LD_PRELOAD implementation with additional clone/vfork hooks
- `crates/ah-command-trace-client/src/lib.rs` - IPC client for sending CommandStart messages
- `crates/ah-command-trace-proto/src/lib.rs` - SSZ protocol definitions and encoding/decoding
- `crates/ah-command-trace-proto/src/messages.rs` - Request/Response message types including CommandStart
- `crates/ah-command-trace-server/src/lib.rs` - Tokio-based async server with shutdown handling
- `crates/ah-command-trace-server/src/test_utils.rs` - TestServer utilities for concurrent testing
- `crates/ah-command-trace-e2e-tests/src/lib.rs` - Test utilities and platform-specific loaders
- `crates/ah-command-trace-e2e-tests/src/bin/spawn_tree.rs` - Test helper binary for creating process trees
- `crates/ah-command-trace-e2e-tests/tests/shim_injection_smoke.rs` - Integration test suite including spawn tree detection

### M1. Sub-Process Detection Harness

**Deliverables**

- [x] Helper binary (`tests/helpers/spawn_tree.rs`) that:
...
- [x] Add test `shim_records_spawn_tree`:
...
- [x] Document handling for short-lived commands, zombies, reparenting, and setuid binaries (shim must skip injection when `AT_SECURE` is set).

**Verification**

- [x] `shim_injection_smoke_basic`: Basic shim loading test (disabled environment)
- [x] `shim_injection_smoke_with_socket`: Full handshake and message exchange test
- [x] `shim_disabled_dormant`: Verifies shim stays dormant when disabled
- [x] `shim_teardown_clean_exit`: Tests clean shutdown on process exit
- [x] `shim_teardown_underscore_exit`: Tests teardown on abnormal exit
- [x] `shim_records_spawn_tree`: Tests subprocess detection and CommandStart IPC
- [x] Stress test: helper loops 200 times creating burst of sub-processes; shim must report all without socket backpressure errors.
- [x] Coverage for both dynamic shells (`/bin/sh`) and direct exec of binaries with custom interpreters (`/usr/bin/env python`).
  - ✅ **Successfully implemented and verified Python subprocess capture**:
    - Fixed `clone()` hook to treat `CLONE_VFORK` as process creation (captures posix_spawn-based subprocesses)
    - Refactored `vfork()` hook to be parent-driven (safer, avoids UB in child)
    - Added comprehensive exec hooks: `execv`, `execveat`, `execvpe` for additional coverage
    - Implemented lazy client connection with retry logic for robust communication
    - Fixed crate configuration: `cdylib` instead of `dylib` for proper shared library generation
    - **Integration tests confirm**: Python interpreter processes AND their subprocesses are captured
    - **Test results**: Both `shim_shell_and_interpreter_coverage` and `shim_stress_test_burst_subprocesses` pass ✅
  - ✅ **Indirect child process interception working**: The tests verify that processes launched by bash and python (indirect children) are properly intercepted
    - Python `subprocess.run()` calls trigger posix_spawn hooks and are captured
    - Shell script subprocesses are tracked through the command execution pipeline
    - Lazy connection establishment ensures reliable communication between shim and server
  - ✅ **Test infrastructure fully restored and functional**:
    - `test_helper shell_and_interpreter` command restored and working
    - `spawn_tree stress_test` command restored and working
    - All test binaries compile and execute correctly in Nix environment

### M2. Stdout/Stderr Chunk Capture

**Deliverables**

- [x] Helper program (`tests/helpers/mixed_output.rs`) that:
...
- [x] IPC protocol now includes `CommandChunk` messages with `stream` (`stdout`/`stderr`), `sequence_no`, byte payload, and optional `pty_offset`.
- [x] Tests asserting:
...
- [x] Integration tests verifying binary diff of captured chunk payloads vs. expected fixture.
- [x] Edge-case tests:
...
- [x] macOS regression (Nov 19 2025): `cargo test -p ah-command-trace-e2e-tests shim_captures_mixed_output` on arm64 verifies stdout/stderr chunk capture outside Linux CI.

**Implementation Details**

M2 implementation successfully captures stdout/stderr streams including complex scenarios:

- **Core Logic**:
  - Extended `ShimState` to include an FD mapping table (`HashMap<i32, StreamType>`) initialized with default stdout/stderr.
  - Added `CommandChunk` message type to IPC protocol for streaming output data.
  - Implemented `send_command_chunk` in the shim with re-entrancy protection (`thread_local` guard) to prevent infinite recursion during logging.

- **System Call Hooks**:
  - `write`: Captures data written to tracked FDs.
  - `writev`: Handles vectored I/O, reconstructing the data stream.
  - `sendmsg`: Captures data sent via socket interface (often used for logging).
  - `dup`/`dup2`/`dup3`/`fcntl`: Tracks FD duplication to ensure redirected streams (e.g., `dup2(1, 7)`) are still captured.
  - `close`: Removes closed FDs from the tracking table.
- **Self-Reporting Initialization (Nov 2025 update)**:
  - Removed unsafe `exec*`/`clone`/`vfork` hooks and replaced them with a **self-reporting** path in `initialize_client()`. Each process now announces itself immediately after connecting to the recorder (PID/PPID, argv/env snapshots, cwd, executable) removing the async-signal-safety hazards seen earlier.
  - Connection attempts became **fail-open**: we try once, mark the shim state as `Error` on failure, and allow the target workload to continue without blocking. This eliminated the “dogpile” that previously left Python subprocesses stuck in uninterruptible sleep states.
  - Added helper utilities inside `crates/ah-command-trace-shim/src/posix.rs` to collect binary-safe argv/env/cwd data, ensuring the emitted `CommandStart` frames match recorder expectations.
- **macOS Interpose Support (Nov 2025 update)**:
  - Vendored `redhook` into `crates/stackable-interpose` and updated every hook definition to use that crate. The macOS path now emits explicit `__DATA,__interpose` entries marked `#[used]`, preventing `-dead_strip` from dropping our trampolines and finally unlocking stdout/stderr chunk capture on macOS while keeping Linux behavior unchanged.
  - Restricted interposition of variadic libc entry points (e.g., `fcntl`) to Linux until we can model their pointer arguments safely on macOS. The rest of the FD lifecycle (`write*`, `sendmsg`, `dup*`, `close`) now behaves identically on both platforms.
  - Confirmed the new path end-to-end by running `cargo test -p ah-command-trace-e2e-tests shim_captures_mixed_output` on Apple Silicon hardware (Nov 19 2025).
- **Test & Spec Alignment**:
  - `shim_shell_and_interpreter_coverage` now passes consistently (validated with `cargo test -p ah-command-trace-e2e-tests shim_shell_and_interpreter_coverage -- --nocapture`), confirming both shell-launched and Python-launched subprocesses self-report correctly.
  - Documented the rationale and architecture in `specs/Public/Command-Execution-Tracing.md`, covering self-reporting, fail-open connection strategy, artifact management, and verification flow.

- **Testing**:
  - Created `mixed_output` helper binary that exercises all capture paths: single byte writes, large chunks, `writev`, `dup2` redirection, ANSI codes, and binary data.
  - Added `shim_captures_mixed_output` integration test verifying that all output patterns are correctly captured, attributed to the right stream (stdout/stderr), and preserve data integrity (binary exact matches).
...
- [ ] Scenario variant stressing multiple concurrent tools (multi-agent agents) to ensure per-task isolation.
