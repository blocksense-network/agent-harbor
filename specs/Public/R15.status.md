...
- [x] Unit tests for configuration merging and precedence (system/user/repo/repo-user/env/flags) plus provenance reporting.
...
- [x] Help/JSON snapshot tests to lock the updated flag set (`cargo test -p ah-cli --test cli_help_snapshots` extension).
...
- [x] Unit tests for prompt processing (template stripping, comment handling, empty detection) and branch validation (regex, primary protection, follow-up detection).
- [x] Tests for metadata commit generation and `.agents/tasks` path/content (including follow-up delimiter, dev-shell line, remote discovery).
- [x] Scenario tests under `tests/tools/mock-agent/scenarios/` covering new task creation vs. follow-up, `--create-task-files` toggles, and push prompt behaviors.
- Notes: Added coverage for editor-hint stripping, inline hash preservation, git `core.commentString` overrides, follow-up branch guards, and scenario fixtures for new/follow-up/metadata-only/push flows.
...
- [x] Integration tests that drive `ah task` through local and remote code paths, asserting workspace selection records, task state persistence, `.agents/tasks` updates, and metadata commits.
- [x] Scenario tests for multi-agent and multi-instance launches plus fleet orchestration (local leader + remote member stubs) with mock agent recordings.
- [x] Regression test ensuring cloud-agent/browser automation flags are rejected with clear messaging.
- [x] Integration test that `--follow` opens the correct monitoring UI and attaches to the new session/task ID.
...
- [x] Integration tests for delivery modes (branch/pr/patch) covering push prompts, `--push-to-remote` automation, and failure rollback.
- [x] Unit tests for branch target validation and push option parsing (including non-interactive exit 10 without `--push-to-remote`).
- [x] Scenario-driven tests confirming notifications flag propagation to spawned processes (validated via mock notifier hooks).
...
- [x] `just lint-rust` and `just test-rust` remain green with new coverage.
- [x] Manual test script runs without intervention following the documented steps; logs stored per run for troubleshooting.
- [x] Documentation spell-check/link-check passes where applicable.
