...
- Introduce an `Autocomplete Active` minor mode that surfaces a dedicated `Accept Autocomplete` operation (Tab by default) so inline completions take precedence over form navigation whenever a suggestion is present.
...
- Ensure the PRD input suite covers both acceptance paths (Tab-driven shared extension vs. full completion) and asserts that Tab falls back to next-field navigation as soon as no suggestion is available.
...
- Ensure draft textarea focus shows `Enter Launch Agent(s) • Shift+Enter New Line • Tab Complete/Next Field`, and other states follow the PRD matrix.
