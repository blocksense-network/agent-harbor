...
**Latest run – 2025-11-21:** Full harness rerun on branch `feat/agentfs-fuse-f7`. F1–F5, F7, F8, F9, and F10 all passed (logs under `logs/fuse-*-20251121-*`), including `just test-pjdfstest-full` with only the expected privileged `chmod/12.t` failures. F6 performance still below targets even after enabling writeback cache + inline writes and bumping max_write/background (seq_write ~0.36×, seq_read ~0.34×, metadata ~0.21×, concurrent_write ~0.12×) in `logs/fuse-performance-20251121-153312/summary.json`; kernel passthrough was rejected (`EPERM` on `open_backing`, CAP_SYS_ADMIN missing on /dev/fuse).
...
- **Verification Results**:
  - [x] Performance harness – `scripts/test-fuse-performance.sh` (`just test-fuse-performance`) mounts AgentFS with a HostFs backstore, runs sequential read/write, metadata, and 4-way concurrent write benchmarks against a host baseline, and emits structured logs (`results.jsonl` + `summary.json`). Latest run uses release host binary by default, writeback cache enabled, inline writes (no extra copy/queue), 8 MiB max_write/blocks, and elevated max_background; results remain below thresholds because passthrough was denied (`open_backing` → `EPERM`, CAP_SYS_ADMIN missing): `logs/fuse-performance-20251121-153312/summary.json` (seq_write ≈ 0.36×, seq_read ≈ 0.34×, metadata ≈ 0.21×, concurrent_write ≈ 0.12×).
