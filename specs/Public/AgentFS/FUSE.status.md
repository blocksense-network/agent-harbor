...
- [x] Current compliance status – `logs/pjdfstest-full-20251118-044034/summary.json` shows a completely green run (including the privileged chmod/12 pass) after the inode/tombstone invalidation fixes. `just test-pjdfstest-full` now matches the baseline without deltas, and the historical “all green” claim is finally accurate.
