...
### M1. Control-plane foundations (1–2 weeks)
...
- **Outstanding Tasks**
  - Document run-book steps for the new harness targets in `docs/` and ensure cross-platform detection covers Windows parity (tracked for later milestones).

### M2. Workspace preparation in CLI & sandbox tooling (1 week)
...
- **Deliverables**
  ...
  - Extend `ah agent sandbox` with `--fs-provider` (`auto|agentfs|git|zfs|btrfs`) and `--agentfs-socket` flags that reuse existing daemons when provided.
  - Add telemetry and debug logging (via `ah-logging`) that records provider choice, daemon PID, and mount paths.
- **Verification**
  - [ ] `cargo test -p ah-cli --test sandbox_workspace_agentfs` exercises `ah agent sandbox --fs-provider agentfs` end-to-end, asserting the command exits cleanly, the workspace is isolated, and the daemon is reused when `--agentfs-socket` is provided.
  - [ ] `just lint-rust` remains clean after the logging hooks are added (CI gate).
  - [ ] New snapshot of CLI help (`cargo test -p ah-cli --test cli_help_snapshots`) verifies the documented flags include AgentFS options.
