...
It is implemented as a **static documentation site** using **Next.js 16 + Nextra 4 (Docs theme)** with filesystem-based routing under `docs/site/app`. Content is authored in Markdown/MDX inside the main repository and presented as a navigable, searchable experience.

- `docs/site/app/layout.jsx` composes the Nextra `<Layout>` and `<Navbar>` with `getPageMap` to auto-wire sidebar/page navigation.
- `docs/site/mdx-components.js` merges Nextra theme components with custom MDX components, so bespoke blocks can be added without losing defaults.
