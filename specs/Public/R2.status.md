# R2. Multi-Agent Draft Selectors — Status
...
- Button text still shows only the first agent; pluralization (`"🤖 X agent(s)"`) is tracked under M3.
- Modal inventory is hardcoded and lacks async loading/error states (see M1).
- Docs/config/manual flows must describe the new selector (M4/M5).

### M1. Agent Catalog & Enumerators

**Deliverables**

- [ ] Introduce `AgentCatalog` + `AgentsEnumerator` in `ah-core`, normalizing agent metadata from:
...
- [ ] Shared domain structs (`ah-domain-types`) expose experimental flags, display names, capabilities, and per-agent defaults (model, instance count, settings).
- [ ] TUI, CLI, and REST clients depend on the enumerator instead of hardcoded lists.
- [ ] `ah-cli`/`ah-config-types` gain `default_agents` + `experimental_agents` settings with precedence resolution (env, repo config, user config).
- [ ] `--experimental-features <set>` CLI/config flag gates experimental catalog entries and surfaces friendly warnings when disabled.

**Verification (automated)**

- [ ] `ah-core` unit tests cover catalog merging, deduplication, experimental gating, and REST fallback behavior.
- [ ] `ah-cli` config tests ensure precedence (env > repo > user > defaults) and `--experimental-features` parsing.
- [ ] TUI unit tests mock the enumerator to assert loading/error states propagate to the view-model (`loading_agents`, `agent_errors`).
- [ ] REST contract tests simulate `GET /api/v1/agents` responses (success, transient failure, empty) and ensure enumerator retries obey exponential backoff and TTL invalidation.
...
- [ ] TUI ViewModel tests simulate enumerator futures resolving success/failure and assert UI transitions (loading → success, loading → error → retry).
...
- [ ] Scenario tests (goldens) record the modal states for “loading”, “empty catalog”, “error with retry”.

### M3. UX & Launch Polish

**Deliverables**

- [ ] Update draft card button text to show `"🤖 X agent(s)"`, with tooltip listing the top selections.
- [ ] Modal footer hints adapt to count states (e.g., “Enter Keep Selections • +/- Adjust Count • Esc Cancel”).
- [ ] Implement contextual validation: disable “Go” when selections are empty or exceed configurable per-task limits; show error banners with actionable guidance.
- [ ] Add `AgentSelectorState` to local DB so reopened drafts preserve per-agent count ordering and “last adjusted” metadata.
