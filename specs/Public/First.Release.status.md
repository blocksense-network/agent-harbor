...
- A reproducible Nix package is generated by a GitHub workflow.
- A reseach is ongoing about how to wrap (using `nix-bundle` or similar) and sign that package for other package managers.

**Context:**

- Our CI:
  - Runs tests
  - Builds and caches the `nix` package for Agent Harbor
  - Caching of said package is done with `Cachix`
  As a result, all artifacts are signed, ours by `Cachix` directly, dependencies by `nixpkgs`'s `Hydra` CI.
  We still need an automated pipeline that builds, signs/notarizes, and publishes release artifacts across all other (non-`nix`) platforms with consistent versioning.
- Release documentation/changelog updates require coordination; a formal workflow will reduce regressions.

**Deliverables:**

- [x] Create a GitHub Actions workflow that triggers on every PR that build the core `nix` package, uploading it to `Cachix` on completion.
- [ ] Create a GitHub Actions workflow triggered on tags (and optional dry runs) that builds platform packages (by wrapping the aforementioned `nix` package), performs signing/notarization, and uploads artifacts to GitHub Releases (and future registries).
- [ ] Generate checksums/SBOMs and verify artifacts before publish.
  - Will involve tooling like [sbomnix](https://github.com/tiiuae/sbomnix) and [genealogos](https://github.com/tweag/genealogos)
