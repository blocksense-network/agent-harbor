...
async fn test_get_initial_tasks() {
    ...
    // Check tasks - MockTaskManager returns 4 tasks
    assert_eq!(tasks.len(), 4);
    assert_eq!(tasks[0].id, "task_001");
    assert_eq!(tasks[0].title, "Implement user authentication system");
    assert_eq!(tasks[0].status, "running");
    assert_eq!(tasks[1].id, "task_002");
    assert_eq!(tasks[1].title, "Set up CI/CD pipeline");
    assert_eq!(tasks[1].status, "running");
    assert_eq!(tasks[2].id, "task_003");
    assert_eq!(tasks[2].title, "Add database migrations");
    assert_eq!(tasks[2].status, "completed");
    assert_eq!(tasks[3].id, "task_004");
    assert_eq!(tasks[3].title, "Refactor API endpoints");
