...
impl InlineAutocomplete {
    ...
    pub fn handle_key_event(&mut self, key: &KeyEvent, textarea: &mut TextArea<'_>) -> bool {
        if !self.vm.open {
            return false;
        }
        ...
        match key.code {
            ...
            KeyCode::Down => {
                if !self.vm.results.is_empty() {
                    self.vm.selected = (self.vm.selected + 1) % self.vm.results.len().max(1);
                }
                true
            }
            KeyCode::PageUp => {
            ...
            KeyCode::Tab | KeyCode::BackTab => {
                self.commit_selection(textarea);
                true
            }
    ...
    pub fn after_textarea_change(&mut self, textarea: &TextArea<'_>) {
        ...
        if let Some((trigger, token, query)) = extract_token(textarea) {
            if self.vm.trigger != Some(trigger) {
                self.vm.selected = 0;
            }
    ...
    pub fn render(
        ...
        let (anchor_x, anchor_y) = caret_anchor(textarea, textarea_area);
        let screen = frame.area();
        let menu_width = MENU_WIDTH.min(screen.width);
        let menu_height = min(self.vm.results.len().max(1) as u16, MAX_MENU_HEIGHT).max(1);
        let popup = clip_popup(screen, anchor_x, anchor_y, menu_width, menu_height);
        ...
        let items: Vec<ListItem> =
            self.vm.results.iter().map(|m| make_list_item(m, theme)).collect();
        ...
        state.select(Some(self.vm.selected));
    ...
    fn commit_selection(&mut self, textarea: &mut TextArea<'_>) {
        if self.vm.results.is_empty() {
            self.close();
            return;
        }
...
fn caret_anchor(textarea: &TextArea<'_>, area: Rect) -> (u16, u16) {
    let (row, col) = textarea.cursor();
    let (top, left) = textarea.viewport_origin();
    let gutter = textarea.gutter_width();

    let visible_row = row.saturating_sub(top as usize) as u16;
    let visible_col = col.saturating_sub(left as usize) as u16;

    let anchor_y = area.y.saturating_add(visible_row).saturating_add(1);
    let anchor_x = area.x.saturating_add(gutter).saturating_add(1).saturating_add(visible_col);
...
fn clip_popup(area: Rect, x: u16, y: u16, w: u16, h: u16) -> Rect {
    ...
    let width = w.min(area.x + area.width - clamped_x);
    let height = h.min(area.y + area.height - clamped_y);
    Rect {
        x: clamped_x,
        y: clamped_y,
        width,
