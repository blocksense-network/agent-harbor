...
// Interposed functions using stackable-interpose
stackable_interpose::hook! {
    unsafe fn open(stackable_self, path: *const c_char, flags: c_int, mode: mode_t) -> c_int => my_open {
        ...
        stackable_interpose::call_next!(stackable_self, open, path, flags, mode)
    }
}
...
stackable_interpose::hook! {
    unsafe fn read(stackable_self, fd: c_int, buf: *mut c_void, count: size_t) -> ssize_t => my_read {
        ...
        // Call original function
        stackable_interpose::call_next!(stackable_self, read, fd, buf, count)
    }
}
...
stackable_interpose::hook! {
    unsafe fn write(stackable_self, fd: c_int, buf: *const c_void, count: size_t) -> ssize_t => my_write {
        ...
        // Call original function
        stackable_interpose::call_next!(stackable_self, write, fd, buf, count)
    }
}
