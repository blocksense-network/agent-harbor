...
- **Filesystem Interposition**: Two protocol-specific implementations:
  ...
  - **Rust Implementation**: Uses SSZ protocol on `.ssz` socket with the in-tree `stackable_interpose` macros (redhook-derived) for safe function hooking
- **RPC Protocols**:
...
- `rust-client/Cargo.toml` - Rust client dependencies (SSZ, stackable-interpose, AgentFS)
