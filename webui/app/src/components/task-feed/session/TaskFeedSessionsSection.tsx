        ...
        <div class={draftsCount() > 0 ? 'mt-6' : ''}>
          <ul role="listbox" class="space-y-3">
            <For each={props.sessions()}>
              {(session, index) => {
                const globalIndex = draftsCount() + index();
                const isKeyboardSelected = props.keyboardSelectedIndex() === globalIndex;
                const isExplicitSelected = selectedSessionId() === session.id;
                const isSelected = isKeyboardSelected || isExplicitSelected;
                return (
                  <li id={`task-${session.id}`} role="option" aria-selected={isSelected}>
                    <SessionCard
                      session={session}
                      isSelected={isSelected}
                      onClick={() => props.onSelectSession(session.id, globalIndex)}
