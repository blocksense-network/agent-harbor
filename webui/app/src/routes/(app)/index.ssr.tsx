...
export const route = {
  load: async () => {
    // Skip server function preloading in CSR/client-only mode
    // In CSR builds (like Electron), server functions don't exist
    // The component will fetch data directly via the API client instead
    if (typeof window !== 'undefined' && !import.meta.env.SSR) {
      logger.log('[Route] Skipping server function preload (CSR mode)');
      return;
    }

    // SSR mode: Preload both to ensure data is cached before component renders
    logger.log('[Route] Preloading data...');
    const [sessions, drafts] = await Promise.all([getSessionsData(), getDraftsData()]);
